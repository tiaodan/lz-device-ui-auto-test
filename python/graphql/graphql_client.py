...
class GraphQLClient:
    ...
    # Token 有效期（秒），默认 7 天（从 expLen 计算）
    ...
    def __init__(self, base_url: str, login_url: str = "", username: str = "", password: str = ""):
        ...
        Args:
            base_url: GraphQL API 地址，如 https://192.168.85.238/rf/graphql
            login_url: 登录接口地址，如 https://192.168.85.238/login
            username: 用户名
            password: 密码
        """
                ...
                with open(cache_path, "r") as f:
                    ...
                    if cache.get("login_url") == self.login_url:
                        ...
                        if self.token_expire_time and time.time() < self.token_expire_time:
                            ...
                            print(f"[GraphQL] 使用缓存 Token")
                            return True
                        else:
                            print(f"[GraphQL] 缓存 Token 已过期")
            except Exception as e:
                print(f"[GraphQL] 加载缓存失败: {e}")
        return False
    ...
    def _save_token_cache(self):
        ...
        try:
            ...
            with open(cache_path, "w") as f:
                json.dump(cache, f)
            print(f"[GraphQL] Token 已缓存")
        except Exception as e:
            print(f"[GraphQL] 缓存保存失败: {e}")
    ...
    def login(self, username: str, password: str) -> Dict[str, Any]:
        ...
        使用 HTTP POST 到 /config 接口，发送 JSON 格式参数
        """
        self.username = username
        self.password = password
        ...
        try:
            ...
            if result.get("token"):
                ...
                print(f"[GraphQL] 登录成功，Token 有效期 {int(exp_len)} 秒")
                return result
            else:
                print(f"[GraphQL] 登录失败: {result.get('msg', result)}")
                return result

        except Exception as e:
            print(f"[GraphQL] 登录异常: {e}")
            ...
            if result.get("errors"):
                for error in result["errors"]:
                    ...
                    if "unauthorized" in error_msg or "token" in error_msg or "expired" in error_msg:
                        print("[GraphQL] Token 可能过期，尝试重新登录")
                        self._clear_cache()
                        if self.username and self.password:
    ...
    def get_events_aggregation(
        ...
        """事件聚合查询"""
        query = EVENTS_AGGREGATION_QUERY.format(
            offset=offset,
            limit=limit,
            begin_time=begin_time,
            end_time=end_time,
    ...
    def add_blacklist(
        ...
        """添加黑名单"""
        query = ADD_BLACKLIST_MUTATION.format(
            drone_id=drone_id,
            drone_type=drone_type,
            alias=alias
        )
    ...
    def add_whitelist(
        ...
        drone_type: str,
        alias: str = "",
        timerange: str = "permanent,permanent"
    ) -> Dict[str, Any]:
        """添加白名单"""
        query = ADD_WHITELIST_MUTATION.format(
            drone_id=drone_id,
            drone_type=drone_type,
            alias=alias,
            timerange=timerange
    ...
    def ctrl_landing(self, drone_id: str, enable: bool) -> Dict[str, Any]:
        """迫降"""
        query = CTRL_LANDING_MUTATION.format(
            drone_id=drone_id,
    ...
    def set_auto_attack(
        ...
        dir_wb_enabled: bool = False,
        spf_enabled: bool = False,
        protect_white: bool = False
    ) -> Dict[str, Any]:
        """设置无人值守"""
        query = AUTO_ATTACK_MUTATION.format(
            on=str(on).lower(),
            sa_enabled=str(sa_enabled).lower(),
            wb_enabled=str(wb_enabled).lower(),
            dir_wb_enabled=str(dir_wb_enabled).lower(),
            spf_enabled=str(spf_enabled).lower(),
            protect_white=str(protect_white).lower()
        )
        return self._execute(query)
    ...
    def wideband_attack(
        ...
        """宽频干扰"""
        query = WIDEBAND_ATTACK_MUTATION.format(
            band=band,
            wb_status=str(wb_status).lower(),
            gain=gain,
            direction=direction,
            elevation=elevation,
    ...
    def directional_attack(
        ...
        """定向打击"""
        query = DIRECTIONAL_WB_ATTACK_MUTATION.format(
    ...
    def spoofer_switch(
        ...
        """诱骗设置
        action: 0-关闭, 1-打开, 2-迫降, 3-驱离
        """
        query = SPOOFER_SWITCH_MUTATION.format(
            spoofer_id=spoofer_id,
            action=action,
            timeout=timeout,
            direction=direction
        )
    ...
    def set_detecting_band(self, band: str, enable: bool) -> Dict[str, Any]:
        """检测频段开关"""
        query = DETECTING_BANDS_MUTATION.format(
            band=band,
            enable=str(enable).lower()
        )
        return self._execute(query)
    ...
    def delete_events_by_id(
        ...
        """根据ID删除事件"""
        query = DELETE_EVENTS_BY_ID_MUTATION.format(
            drone_id=drone_id,
            drone_type=drone_type,
            all=str(all).lower()
        )
    ...
    def withdraw_false_alarm(self, drone_type: str, drone_id: str) -> Dict[str, Any]:
        """还原误报"""
        query = WITHDRAW_FALSE_ALARM_MUTATION.format(
            drone_type=drone_type,
            drone_id=drone_id
        )
        return self._execute(query)
    ...
    def tdoa_track(self, drone_id: str, enable: bool) -> Dict[str, Any]:
        """TDOA跟踪"""
        query = TDOA_TRACK_MUTATION.format(
            drone_id=drone_id,
            enable=str(enable).lower()
        )
        return self._execute(query)
    ...
    def tracking(self, device_id: str, drone_uuid: str) -> Dict[str, Any]:
        """视觉跟踪"""
        query = TRACKING_MUTATION.format(
            device_id=device_id,
    ...
    def stage_reset(
        ...
        """定向复位"""
        query = STAGE_RESET_MUTATION.format(
            jammer_id=jammer_id,
            direction=direction,
            elevation=elevation,
            track_enable=str(track_enable).lower()
        )
        return self._execute(query)
    ...
    def upgrade_to(self, version_name: str, upgrade_by: str = "manual") -> Dict[str, Any]:
        """版本升级"""
        query = UPGRADE_TO_MUTATION.format(
            version_name=version_name,
            upgrade_by=upgrade_by
        )
        return self._execute(query)
